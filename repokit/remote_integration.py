...
class RemoteIntegration:
    ...
    def __init__(self, repo_manager, credentials_file: Optional[str] = None, verbose: int = 0):
        """
    ...
    def create_github_repository(self, repo_name: str, description: str = "", private: bool = False, 
                              organization: Optional[str] = None, token_command: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        if not token:
            return False, "GitHub token not found. See Authentication Guide for setup instructions."
        ...
        try:
            ...
            try:
                ...
                default_branch_response = requests.patch(default_branch_url, headers=headers, json=default_branch_data)
    ...
    def create_gitlab_repository(self, repo_name: str, description: str = "", private: bool = False,
                              group: Optional[str] = None, token_command: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        if not token:
            return False, "GitLab token not found. See Authentication Guide for setup instructions."
    ...
    def add_remote(self, remote_url: str, remote_name: str = "origin") -> bool:
        ...
        try:
            ...
            if existing_remote:
                ...
                self.repo_manager.run_git(["remote", "set-url", remote_name, remote_url])
    ...
    def push_to_remote(self, branch_name: str, remote_name: str = "origin", 
        ...
        try:
            ...
            if current_branch != branch_name:
                self.logger.warning(f"Not on branch {branch_name}, currently on {current_branch}")
                ...
                push_cmd = ["push", remote_name, f"refs/heads/{branch_name}:refs/heads/{branch_name}"]
            else:
    ...
    def push_all_branches(self, exclude_branches: Optional[List[str]] = None, 
                        remote_name: str = "origin") -> Tuple[List[str], List[str]]:
        """
                ...
                if self.is_branch_checked_out(branch) and branch != current_branch:
                    ...
                    if remote_check:
                        ...
                        self.repo_manager.run_git(
                            ["push", "-u", remote_name, f"refs/heads/{branch}:refs/heads/{branch}"],
                            check=True
                        )
                else:
        ...
        if successful_branches:
            self.logger.info(f"Successfully pushed branches: {', '.join(successful_branches)}")
        
        if failed_branches:
            self.logger.warning(f"Failed to push branches: {', '.join(failed_branches)}")
    ...
    def is_branch_checked_out(self, branch: str) -> bool:
        ...
        try:
            # Get all worktrees
            worktree_output = self.repo_manager.run_git(["worktree", "list"], check=False)
    ...
    def setup_remote_repository(self, service: str = "github", remote_name: str = "origin",
                              private: bool = False, push_branches: bool = True,
                              organization: Optional[str] = None,
                              token_command: Optional[str] = None) -> bool:
        """
        ...
        if push_branches:
            ...
            if failed:
                self.logger.warning(f"Some branches could not be pushed: {', '.join(failed)}")
                return False
